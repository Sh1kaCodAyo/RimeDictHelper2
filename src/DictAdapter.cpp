...
bool LoadBaseDict(const std::wstring &filePath) {
  ...
  OutputDebugString(
      (L"单字数: " + std::to_wstring(g_charCodeMap.size()) + L"\n").c_str());
  OutputDebugString((L"========================================\n"));
...
void getCode(HWND hWnd) {
  ...
  if (len == 1) {
    ...
    codesw = codesw1.substr(0, 1) + codesw2.substr(0, 1) +
             codesw3.substr(0, 1) + codesw4.substr(0, 1);
  }
  ...
  if (count == 0) {
    ...
    SetStatusText(
        hWnd, (L"当前编码有 " + std::to_wstring(count) + L" 个词条").c_str());
  }
...
bool LoadDictFile(const std::wstring &filePath, int source) {
  ...
  int lineNumber = 0;  // 行号计数器，从0开始

  while (fgetws(line, 1024, fp) != nullptr) {
    lineNumber++;  // 每解析一行有效记录，行号+1
    // 去掉行尾换行符
    size_t len = wcslen(line);
    if (len > 0 && (line[len - 1] == L'\n' || line[len - 1] == L'\r')) {
      line[len - 1] = L'\0';
...
void UpdateConflictList(HWND hListView, const std::wstring &code) {
  ...
  for (const auto &entry : it->second) {
    ...
    std::wstring lineNumberStr = std::to_wstring(entry.lineNumber);
