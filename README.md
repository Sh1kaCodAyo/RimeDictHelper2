...
傻瓜式的，直接用。如果是五笔用户，直接输入新词后会自动生成编码，回车即可。我的习惯是通过Windows快捷方式设定全局快捷键，随时调出使用。


其实从Rime词库的原理上讲，本工具并不针对具体输入模式。但因为我本身是五笔用户，所以针对五笔输入法做了深度适配，包括但不限于：

+ 输入词组后会按照五笔词组编码规则自动生成编码，但可以手动修改。本意是可以自定义特殊符号如emoji的编码
+ 编码一栏最大长度限定为4码
+ 词语最大长度为10。其实五笔输入法很少会有那么长的词，大多还是2~4字。但拼音输入法我不太确定

总之脑补对拼音用户不是非常友好，但也许也能用
